	...
	static void actual_cancer(Calc& calc, const int& hi)
	{
		for (auto itv = 0; itv < calc.numitv; ++itv) {
			...
			calc.soap.at(hi).at(NPSBase).at(itv) =
			  static_cast<float>(notes) * finalscaler * P(1.6F);
...
struct techyo
{
					  ...
					  Calc& calc)
	{
		...
		const auto oioi = P(0.5F);
		pineapple = std::clamp(pineapple + oioi, oioi, P(1.F) + oioi);
		porcupine = std::clamp(porcupine + oioi, oioi, P(1.F) + oioi);
		sequins = std::clamp(sequins + oioi, oioi, P(1.F) + oioi);
	  ...
	  -> float
	{
		...
		return std::max(weighted_average(get_tc_base(calc), nps_base, P(4.F), P(9.F)),
						rm_itv_max_diff);
	}
