...
#pragma region params
	...
	float min_mod = 0.240207F;
	float max_mod = 0.994918F;
	float base = 1.46923F;
	float suppression = 0.204527F;

	float cv_reset = 0.936378F;
	float cv_threshhold = 0.222152F;

	float min_len = 7.92744F;
...
#pragma endregion
	...
	auto make_thing(const float& itv_taps) -> float
	{
		...
		return std::clamp(hello_my_name_is_goat, 0.0953646F, 0.881857F);
	}
	...
	{
		moving_cv = (moving_cv + ms_any.get_cv_of_window(cc_window)) / 2.0215F;
