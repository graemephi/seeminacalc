...
#pragma region params
	...
	float min_mod = 1.0036F;
	float max_mod = 1.04549F;
	float base = 0.515347F;
	float min_anchor_len = 5.01684F;
	float min_taps_in_rm = 1.03149F;
	float min_off_taps_same = 1.07742F;

	float offhand_tap_prop_scaler = 0.967229F;
	float offhand_tap_prop_min = -0.F;
	float offhand_tap_prop_max = 1.01711F;
	float offhand_tap_prop_base = 1.76389F;

	float off_tap_same_prop_scaler = 1.08869F;
	float off_tap_same_prop_min = -0.F;
	float off_tap_same_prop_max = 1.32191F;
	float off_tap_same_prop_base = 0.743668F;

	float anchor_len_divisor = 4.71658F;
	float anchor_len_comp_min = 0.F;
	float anchor_len_comp_max = 1.25F;

	float min_jack_taps_for_bonus = 0.992748F;
	float jack_bonus_base = 0.0981604F;

	float min_oht_taps_for_bonus = 0.943248F;
	float oht_bonus_base = 0.100733F;

	// params for rm_sequencing, these define conditions for resetting
	// runningmen sequences
	float max_oht_len = 1.96019F;
	float max_off_len = 3.17493F;
	float max_ot_sh_len = 2.05594F;
	float max_burst_len = 6.01305F;
	float max_jack_len = 2.79472F;
	float max_anch_len = 4.84301F;
...
#pragma endregion params and param map
	...
	[[nodiscard]] auto get_highest_anchor_difficulty() const -> float
	{
		...
		float oht_p = 1.46661F - (highest_rm._rm.get_offhand_tap_prop() *
							  offhand_tap_prop_scaler);

		oht_p = std::clamp(oht_p, 0.721479F, 0.92758F);
