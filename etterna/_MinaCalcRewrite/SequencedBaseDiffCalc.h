	...
	static void actual_cancer(Calc& calc, const int& hi)
	{
		for (auto itv = 0; itv < calc.numitv; ++itv) {
			...
			calc.soap.at(hi).at(NPSBase).at(itv) =
			  static_cast<float>(notes) * finalscaler * 1.74245F;
...
struct techyo
{
					  ...
					  Calc& calc)
	{
		...
		const auto oioi = 0.537389F;
		pineapple = std::clamp(pineapple + oioi, oioi, 0.995232F + oioi);
		porcupine = std::clamp(porcupine + oioi, oioi, 1.02057F + oioi);
		sequins = std::clamp(sequins + oioi, oioi, 0.966814F + oioi);
		...
		obliosis = std::clamp(obliosis, 1.03156F, 9.76485F);
		auto pewp = fastsqrt(div_high_by_low(scoliosis, poliosis) - 1.F);

		pewp /= obliosis;
		const auto vertebrae = std::clamp(
		  ((pineapple + porcupine + sequins) / 3.F) + pewp, oioi, 1.00848F + oioi);
	  ...
	  -> float
	{
		...
		return std::max(weighted_average(get_tc_base(calc), nps_base, 4.08301F, 9.24491F),
						rm_itv_max_diff);
	}
